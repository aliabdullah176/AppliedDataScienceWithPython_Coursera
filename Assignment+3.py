...
# the [pandas documentation](http://pandas.pydata.org/pandas-docs/stable/) to find
...
# ### Question 1 (20%)
# Load the energy data from the file `Energy Indicators.xls`, which is a list of indicators of 
# [energy supply and renewable electricity production](Energy%20Indicators.xls) from the 
...
# Keep in mind that this is an Excel file, and not a comma separated values file. Also, make sure 
# to exclude the footer and header information from the datafile. The first two columns are unneccessary, 
# so you should get rid of them, and you should change the column labels so that the columns are:
# 
# `['Country', 'Energy Supply', 'Energy Supply per Capita', '% Renewable']`
# 
# Convert `Energy Supply` to gigajoules (there are 1,000,000 gigajoules in a petajoule). For all countries
...
# Power Technology](http://www.scimagojr.com/countryrank.php?category=2102) from the file `scimagojr-3.xlsx`,
# which ranks countries based on their journal contributions in the aforementioned area. Call this
# DataFrame **ScimEn**.
...
def nrg():
    energy = pd.read_excel('Energy Indicators.xls', header = 0, skiprows =17, skip_footer = 38, na_values = '...', usecols = [2,3,4,5])
...
def answer_one():
    ...
    temp1 = pd.merge(temp, ScimEn, how='inner', left_index=True, right_index=True)
    
    temp2 = temp1[['Rank', 'Documents', 'Citable documents', 'Citations', 'Self-citations',
            'Citations per document', 'H index', 'Energy Supply',
            'Energy Supply per Capita', '% Renewable', '2006', '2007', '2008',
            '2009', '2010', '2011', '2012', '2013', '2014', '2015']]
    temp2.head(1)
    temp2 = temp2[temp2.Rank < 16]
    return temp2
answer_one()
...
def answer_two():
    ...
    GDP = gdp()
    ScimEn = sci()
    temp0 = pd.merge(pd.merge(GDP, energy, how='outer', left_index=True, right_index=True),ScimEn, how='outer', left_index=True, right_index=True)
    temp1 = pd.merge(pd.merge(GDP, energy, how='inner', left_index=True, right_index=True),ScimEn, how='inner', left_index=True, right_index=True)
    return (len(temp0.index)-len(temp1.index))
answer_two()
...
def answer_five():
    Top15 = answer_one()
    temp = Top15['Energy Supply per Capita'].mean()
    return temp
answer_five()
...
def answer_seven():
    ...
    Top15 = Top15.sort_values(by='ratio', ascending = False).iloc[0]
    return (Top15.name, Top15.ratio)
answer_seven()
...
def answer_nine():
    Top15 = answer_one()
    Top15['popest'] = Top15['Energy Supply']/Top15['Energy Supply per Capita']
    Top15['citest'] = Top15['Citable documents']/Top15['popest']
    temp = Top15[['Energy Supply per Capita','citest']]
    return temp.corr().iloc[1,0]
answer_nine()
...
# displays the sample size (the number of countries in each continent bin), and the sum, mean, and std deviation
...
def answer_eleven():
    ...
    Top15['PopEst'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    temp = Top15.groupby('Continent').agg({'PopEst': { 'size':np.size, 'sum': np.sum, 'mean': np.mean, 'std': np.std} })
...
def answer_twelve():
    ...
    temp = Top15.groupby(['Continent', 'bins']).size()
    return temp
...
# 
# *This function should return a Series `PopEst` whose index is the country name and
# whose values are the population estimate string.*
